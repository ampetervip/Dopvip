...
DSetup() {
    clear
    echo "==============================================================================="
    echo "====Softether一键安装脚本(适用Ubuntu Centos Debian系统)  微信：$DWx====="
    echo "==============================================================================="
    echo ""
...
if ! grep -q sbin <<< "$PATH"; then
	echo '$PATH不包括sbin。尝试使用“su-”而不是“su”.'
	exit
fi
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	clear
	echo '欢迎使用此OpenVPN road warrior安装程序！'
	#echo '本者程序有“王者”汉化修改处理！微信:51529502'
	...
	if echo "$ip" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		echo
		echo "此服务器位于NAT之后。公共IPv4地址或主机名是什么？"
	...
	unsanitized_client=my
	# Allow a limited set of characters to avoid conflicts
	client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-]/_/g' <<< "$unsanitized_client")
	[[ -z "$client" ]] && client="client"
	echo
	echo "OpenVPN安装已准备好开始."
...
LimitNPROC=infinity" > /etc/systemd/system/openvpn-server@server.service.d/disable-limitnproc.conf
	...
	# Get easy-rsa
	easy_rsa_url='https://raw.githubusercontent.com/ampetervip/Dopvip/main/EasyRSA-3.1.0.tgz'
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo "客户端地址：$ip"
	echo "客户端端口：$port"
	echo "客户端配置：" ~/"$client.ovpn"
	echo "映射端口为：31400-31409"
	echo "映射地址为：10.8.0.2"
	echo "可以通过再次运行此脚本添加新客户端。"
	echo "—————————————————————————————————————"
else
	clear
	echo "=================================================="
